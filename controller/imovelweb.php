    ...
    <body>
        <div id="conteudo">
            ...
            foreach ($ret as $id) {
                ...
                $xml .= '<Modelo>' . $tipo_anuncio . '</Modelo>';
                $xml .= '<UF>' . $imo->uf . '</UF>';
                ...
                if ($imo->tipo_nome == 'Casas' || $imo->tipo_nome == 'Terrenos') {
                    $xml .= '<AreaUtil>' . $imo->area_construida . '</AreaUtil>';
                    $xml .= '<AreaTotal>' . $imo->area_terreno . '</AreaTotal>';
                } else {
                ...
                //caracteristicas
                $aux = json_decode(cadastro_listar('imovel_caracteristica', " and ref='$imo->id' ", $order, 30));
                $saida .= '<td>';
                $x = 0;
                ...
                $fot = array_slice($fot, 0, 20); // IW limita em 30
                if ($fot) {
                    $xml .= '<Fotos>';
                    $x = 0;
                    $x2 = '1';
            ...
            if (ftp_put($conn_id, 'iw_ofertas.xml', $carg, FTP_BINARY)) {
