...
<html>
    <head>
        ...
        <script src="../controller/js/spimovel.js"></script>
    </head>
    <body>
        <div id="conteudo">
            <?php
            include '../controller/portal.php';
            $por = json_decode(portal_carregar('SPIMOVEL'));
            //
            echo '<input type="hidden" name="codigo_cliente" id="codigo_cliente">';
            ...
            echo '</table>';
            //
            $ret = json_decode(publicar_listar('SPIMOVEL'));
            $tot = count($ret);
            ...
            echo '</h4>';
            if ($ret) {
                echo '<input type="buttton" value="Publicar" class="botao" onclick="window.open(\'../controller/spimovel.php\',\'_self\');">';
